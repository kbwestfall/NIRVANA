...
class MaNGAAxisymCollate(scriptbase.ScriptBase):
    ...
    def get_parser(cls, width=None):
        ...
        parser.add_argument(
            'dir', type=str,
            help=(
                'Top-level directory with the output from nirvana_manga_axisym or '
                'nirvana_manga_asymdrift.  This script recursively combs through the directory '
                'structure, looking for files produced by the fitting scripts.  The file names '
                'are expected to be "nirvana-manga-axisym-[plate]-[ifu]-[tracer].fits.gz" or '
                '"nirvana-manga-asymdrift-[plate]-[ifu].fits.gz", respectively.  Each '
                'combination of ([plate], [ifu], [tracer]) *must* be unique.  All files must also '
                'be the result of fitting the same parameterization of the axisymmetric model.'
            )
        )
        ...
        parser.add_argument(
            ...
            help=(
                'DAP analysis key used to select the data files.  This is used to select the '
                'relevant extension of the DAPall file for cross-matching.'
            )
        )
        parser.add_argument(
            '--dr', default='MPL-11', type=str,
            help=(
                'The MaNGA data release.  This is only used to automatically construct the MaNGA '
                'DRPall and DAPall file names.'
            )
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            help=(
                'Top-level directory with the MaNGA DAP output.  If not defined, this is set by '
                'the environmental variable MANGA_SPECTRO_ANALYSIS.  This is only used to '
                'automatically construct the MaNGA DRPall and DAPall file names.'
            )
        )
        parser.add_argument(
            '--drpall', default=None, type=str,
            help=(
                'The full path to the MaNGA DRPall file.  If None, the file name is constructed '
                'assuming the default paths.'
            )
        )
        parser.add_argument(
            '--dapall', default=None, type=str,
            help=(
                'The full path to the MaNGA DAPall file.  If None, the file name is constructed '
                'assuming the default paths.'
            )
        )
        parser.add_argument(
            '-f', '--full', default=False, action='store_true',
            help=(
                'Instead of trolling the directory for output files, attempt to find output for '
                'all plateifus in the DAPall file, for both Gas and Stars.'
            )
        )
        parser.add_argument(
        ...
        parser.add_argument(
            '-a', '--asymdrift', default=False, action='store_true',
            help=(
                'Default is to find files resulting from nirvana_manga_axisym.  This option '
                'changes to finding output from nirvana_manga_asymdrift.'
            )
        )
        parser.add_argument(
            '-r', '--max_nr', default=None, type=int,
            help=(
                'Maximum number of samples for radial profiles.  If not provided, set by the '
                'maximum number of samples in the files to collate.  If provided, code still '
                'searches files, but it will use either this number or the maximum number of '
                'samples in the files to collate, whichever is smaller.'
            )
        )
    ...
    def main(cls, args):
        ...
        if args.full:
            ...
            if args.asymdrift:
                plateifu = np.array([
                    '-'.join(f.name.split('.')[0].split('-')[-2:]) for f in files
                ])
                if plateifu.size != np.unique(plateifu).size:
                    raise ValueError(
                        f'Plate-IFUs must be unique for all asymdrift files found in {args.dir}!'
                    )
            else:
                pltifutrc = np.array([
                    '-'.join(f.name.split('.')[0].split('-')[-3:]) for f in files
                ])
                if pltifutrc.size != np.unique(pltifutrc).size:
                    raise ValueError(
                        'Plate-IFUs must be unique for all axisym files for each tracer found in '
                        f'{args.dir}!'
                    )
        ...
        if args.drpall is None or args.dapall is None:
            _drpall_path, _, _, _dapall_path, _ = manga_paths(
                0, 0, dr=args.dr, redux_path=args.redux, analysis_path=args.analysis
            )
            _drpall_file, _, _, _dapall_file, _ = manga_file_names(0, 0, dr=args.dr)
        _drpall_file = (
            (Path(_drpall_path).resolve() / _drpall_file)
        ...
        _dapall_file = (
            (Path(_dapall_path).resolve() / _dapall_file)
        ...
        gas_disk = AxisymmetricDisk(
            rc=func1d_classes[func1d_class_names.index(rc_class[0])](),
            dc=(
                None if dc_class[0] is None
                else func1d_classes[func1d_class_names.index(dc_class[0])]()
            )
        )
        str_disk = AxisymmetricDisk(
            rc=func1d_classes[func1d_class_names.index(rc_class[1])](),
            dc=(
                None if dc_class[1] is None
        ...
        for i, f in enumerate(files):
            ...
            if (
                (args.asymdrift and maxnr == args.max_nr and max_adnr == args.max_nr)
        ...
        indx = (
            np.where(dapall['DAPDONE'])[0] if args.full
    ...
    #        assert galmeta.dapindx == j, 'Bad match'
            ...
            # Add the gas data
            gas_file = (
                f'{nirvana_root}-{galmeta.plateifu}.fits.gz'
                if args.asymdrift else f'{nirvana_root}-{galmeta.plateifu}-Gas.fits.gz'
            )
            ...
            if gas_file.exists():
                with fits.open(gas_file) as hdu:
                    ...
                    if (
                        hdu[0].header[md_keys[0]] != 'AxisymmetricDisk'
                        ...
                        warnings.warn(
                            ...
                            'parameterization as determined by the template file, '
                            f'{ex_files[0]}.  Skipping this file.'
                        )
                        # Toggle flag
                        gas_metadata['QUAL'][i] = gas_disk.gbm.turn_on(
                            hdu[0].header['QUAL'], 'DIFFMOD'
                        )
                    else:
                ...
                # Toggle flag
                gas_metadata['QUAL'][i] = gas_disk.gbm.turn_on(
                    gas_disk.gbm.minimum_dtype()(0), 'NOMODEL'
                )

            # Add the stellar data
            str_file = (
                f'{nirvana_root}-{galmeta.plateifu}.fits.gz'
                if args.asymdrift else f'{nirvana_root}-{galmeta.plateifu}-Stars.fits.gz'
            )
            ...
            if str_file.exists():
                with fits.open(str_file) as hdu:
                    # Confirm the output has the expected model parameterization
                    if (
                        hdu[0].header[md_keys[1]] != 'AxisymmetricDisk'
                        ...
                        # Toggle flag
                        str_metadata['QUAL'][i] = str_disk.gbm.turn_on(
                            hdu[0].header['QUAL'], 'DIFFMOD'
                        )
                    else:
            ...
            if ad_file.exists():
                ...
                ad_metadata['QUAL'][i] = str_disk.gbm.turn_on(
                    str_disk.gbm.minimum_dtype()(0), 'NOMODEL'
                )
        ...
        hdus = [
            ...
            fits.BinTableHDU.from_columns([
                fits.Column(
                    name=n, format=fileio.rec_to_fits_type(gas_metadata[n]),
                    dim=fileio.rec_to_fits_col_dim(gas_metadata[n]), array=gas_metadata[n]
                ) for n in gas_metadata.dtype.names
            ], name='GAS', header=gas_hdr),
            fits.BinTableHDU.from_columns([
                fits.Column(
                    name=n, format=fileio.rec_to_fits_type(str_metadata[n]),
                    dim=fileio.rec_to_fits_col_dim(str_metadata[n]), array=str_metadata[n]
        ...
        if args.asymdrift:
            hdus += [
                fits.BinTableHDU.from_columns([
                    fits.Column(
                        name=n, format=fileio.rec_to_fits_type(ad_metadata[n]),
                        dim=fileio.rec_to_fits_col_dim(ad_metadata[n]), array=ad_metadata[n]
                    ) for n in ad_metadata.dtype.names
                ], name='AD')
            ]
