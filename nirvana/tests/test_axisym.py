...
def test_disk_derivative_nosig():
    ...
    dp = np.array([0.0001, 0.0001, 0.001, 0.001, 0.001, 0.001, 0.0001])
    ...
    vp = np.empty(v.shape+(disk.par.size,), dtype=float)
    p = disk.par.copy()
    ...
    for i in range(disk.par.size):
        assert np.allclose(dv[...,i], fd_dv[...,i], rtol=0., atol=1e-4), \
...
def test_disk_derivative():
    ...
    dp = np.array([0.0001, 0.0001, 0.001, 0.001, 0.001, 0.001, 0.0001, 0.001, 0.0001])
    ...
    vp = np.empty(v.shape+(disk.par.size,), dtype=float)
    sigp = np.empty(v.shape+(disk.par.size,), dtype=float)
    p = disk.par.copy()
    for i in range(disk.par.size):
        _p = p.copy()
        _p[i] += dp[i]
    ...
    for i in range(disk.par.size):
        ...
        assert np.allclose(dsig[...,i], fd_dsig[...,i], rtol=0., atol=3e-3), \
    ...
    for i in range(disk.par.size):
        ...
        assert np.allclose(dsig[...,i], fd_dsig[...,i], rtol=0., atol=1e-4), \
                f'Finite difference produced different sigma derivative for parameter {i+1}!'
...
def test_disk_derivative_bin():
    ...
    vp = np.empty(v.shape+(disk.par.size,), dtype=float)
    sigp = np.empty(v.shape+(disk.par.size,), dtype=float)
    bvp = np.empty(bv.shape+(disk.par.size,), dtype=float)
    bsigp = np.empty(bv.shape+(disk.par.size,), dtype=float)
    p = disk.par.copy()
    for i in range(disk.par.size):
        _p = p.copy()
        _p[i] += dp[i]
    ...
    for i in range(disk.par.size):
        assert np.allclose(dbv[...,i], fd_dbv[...,i], rtol=0., atol=1e-4), \
                f'Finite difference produced different derivative for parameter {i+1}!'
        # The difference is relatively large (again) for the dispersion data
        assert np.allclose(dbsig[...,i], fd_dbsig[...,i], rtol=0., atol=1e-3), \
                f'Finite difference produced different sigma derivative for parameter {i+1}!'
...
def test_disk_derivative_bin_moments():
    ...
    for i in range(disk.par.size):
#        vdiff = np.absolute(dbv[...,i]-fd_dbv[...,i])
#        sdiff = np.absolute(dbsig[...,i]-fd_dbsig[...,i])
#        print(i, np.amax(vdiff), np.amin(vdiff), np.amax(sdiff), np.amin(sdiff))
#        print(i, np.amax(vdiff[kin.nspax > 1]), np.amin(vdiff[kin.nspax > 1]),
#                 np.amax(sdiff[kin.nspax > 1]), np.amin(sdiff[kin.nspax > 1]))
#        continue
        assert np.allclose(dbv[...,i], fd_dbv[...,i], rtol=0., atol=1e-4), \
                f'Finite difference produced different derivative for parameter {i+1}!'
        # The difference is relatively large (again) for the dispersion data
...
#    disk._fit_prep(kin, p0, None, None, True, True, True, None)
    ...
    # Brute force it
    chip = np.empty(dchi.shape, dtype=float)
    p = disk.par.copy()
    for i in range(disk.par.size):
        _p = p.copy()
        _p[i] += dp[i]
    ...
    for i in range(disk.par.size):
#        diff = np.absolute(dchi[...,i]-fd_dchi[...,i])
#        print(i, np.amax(diff), np.amin(diff))
#        continue
        assert np.allclose(dchi[...,i], fd_dchi[...,i], rtol=0., atol=1e-3), \
                f'Finite difference produced different derivative for parameter {i+1}!'
...
def test_lsq_nopsf():
    ...
    rc = HyperbolicTangent(lb=np.array([0., 1e-3]), ub=np.array([500., kin.max_radius()]))
    ...
    assert np.all(np.absolute(disk.par[:2]) < 0.1), 'Center changed'
...
def test_lsq_with_sig():
    ...
    dc = Exponential(lb=np.array([0., 1e-3]), ub=np.array([500., kin.max_radius()]))
    # Set the disk velocity field
...
#    print(f'First fit (no covar): {time.perf_counter()-t} s')
    ...
    assert np.sum(rej) == 21, 'Different number of pixels were rejected'
...
#    print(f'Second fit (w/ covar): {time.perf_counter()-t} s')
    ...
    assert np.sum(rej) == 7, 'Different number of pixels were rejected'
    # Model parameters
    assert np.all(np.absolute(disk.par[:2]) < 0.1), 'Center changed'
    assert 165. < disk.par[2] < 167., 'PA changed'
...
def test_mock_noerr():
    ...
    mock_sremap = mock_kin.remap(np.sqrt(mock_kin.sig_phys2), mask=kin.sig_mask)

    assert np.ma.allclose(mock_vremap, vremap), 'Bad mock velocity'
    assert np.ma.allclose(mock_sremap, sremap), 'Bad mock dispersion'
...
def test_mock_err():
    ...
    assert np.ma.std(mock_vremap-vremap) > 5, 'Velocity error changed'
    assert np.ma.std(mock_sremap-sremap) > 7, 'Dispersion error changed'
