...
def test_convolve():
    ...
    assert np.all(np.isclose(astsynth, intsynth)), 'Difference wrt astropy convolution'
...
def test_beam():
    ...
    assert np.argmax(synth) == np.argmax(_synth), \
...
def test_fft():
    synth = beam.gauss2d_kernel(73, 3.)
    synth_fft = np.fft.fftn(np.fft.ifftshift(synth))
    ...
    assert np.allclose(synth2, _synth2), 'Difference if FFT is passed for numpy'

    # Compare numpy and FFTW with direct input
    _synth2 = _convolve_fft(synth, synth)
    assert np.allclose(synth2, _synth2), 'Difference between numpy and FFTW'

    # Compare FFTW with direct vs. FFT kernel input
    synth2 = _convolve_fft(synth, synth_fft, kernel_fft=True)
    assert np.allclose(synth2, _synth2), 'Difference if FFT is passed for FFTW'
...
def test_smear():
    ...
    r, theta = projected_polar(x, y, *np.radians([45., 30.]))

    rc = oned.HyperbolicTangent(par=np.array([100., 1.]))
    sig = oned.Exponential(par=np.array([100., 20.]))
    sb = oned.Sersic1D(par=np.array([1., 10., 1.]))

    sb_field = sb.sample(r)
    vel_field = rc.sample(r)*np.cos(theta)
    ...
    assert np.allclose(vel_smear, _vel_smear), 'Velocity-field-only convolution difference.'

    vel_smear = beam.smear(vel_field, synth)[1]
    _vel_smear = beam.smear(vel_field, synth_fft, beam_fft=True)[1]
    assert np.allclose(vel_smear, _vel_smear), \
            'Velocity-field difference w/ vs. w/o precomputing the beam FFT for np.'
    ...
    assert np.allclose(sb_smear, _sb_smear), 'SB+Vel convolution difference in SB.'
    assert np.allclose(vel_smear, _vel_smear), 'SB+Vel convolution difference in Vel.'
    ...
    _sb_smear, _vel_smear, _sig_smear = beam.smear(vel_field, synth, sb=sb_field, sig=sig_field,
                                                   cnvfftw=cnvlv)
    assert np.allclose(sb_smear, _sb_smear), 'SB+Vel+Sig convolution difference in SB.'
    assert np.allclose(vel_smear, _vel_smear), 'SB+Vel+Sig convolution difference in vel.'
    assert np.allclose(sig_smear, _sig_smear), 'SB+Vel+Sig convolution difference in sig.'
...
def test_deconvolve():
    ...
    assert np.sum((c-dc)**2) < 1e-9, 'Deconvolution performance worsened.'
...
#    test_c = cnvfftw(d, synth)
    ...
    # Convolve the deconvolved image with the kernel to compare with original image
    d_gpm = np.logical_not(d_bpm).astype(float)
    cnv_gpm = cnvfftw(d_gpm, synth)
    test_sb = cnvfftw(d * d_gpm, synth) / (cnv_gpm + (cnv_gpm == 0.)) * d_gpm

    # Test total flux recovery
    obs_flux = np.sum(kin.grid_sb * d_gpm)
    model_flux = np.sum(test_sb)
    assert np.absolute(model_flux / obs_flux - 1) < 1e-4, 'Change in total flux too large'

    # Test residual
    model_rms = np.sqrt(np.mean(np.square((kin.grid_sb - test_sb)*d_gpm)))
...
def test_deconvolve_star():
    ...
    obs_flux = np.sum(kin.grid_sb * d_gpm)
    model_flux = np.sum(test_sb)
    assert np.absolute(model_flux / obs_flux - 1) < 1e-3, 'Change in total flux too large'
